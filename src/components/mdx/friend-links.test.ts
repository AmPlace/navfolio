...
describe('friend link data helpers', () => {
  ...
  test('normalizes malformed direct items without throwing', () => {
    const [item] = normalizeFriendLinkItems([
      {
  ...
  test('preserves string content that looks like a trailing comma', async () => {
    const resolved = await resolveFriendLinkItems(
      {
        src: 'https://example.com/friends.json',
      },
          ...
          JSON.stringify([
            {
              ...
              bio: 'I like commas, } and stuff',
            },
          ]),
        ),
    );

    expect(resolved[0]?.bio).toBe('I like commas, } and stuff');
  });

  test('rejects remote JSON with trailing commas instead of rewriting it', async () => {
    const load = () =>
      resolveFriendLinkItems(
        {
          src: 'https://example.com/friends.json',
        },
        async () => new Response('[{"name":"Trailing","url":"https://example.com/trailing",}]'),
      );
